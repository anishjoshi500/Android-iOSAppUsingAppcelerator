...
//Rare exceptions to indicate a bug in the _jester code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_jester = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_jester]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.jester.module"
